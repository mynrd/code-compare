...
async function log(root, opts = {}) {
  ...
  const scanLimit = Math.max(limit, DEFAULT_LOG_LIMIT);
  const unfiltered = parseLog(
    (await runText(root, buildLogArgs({ ...base, grep: '', skip: 0, limit: scanLimit }))).stdout
  );
  const prefix = grep.toLowerCase();
  const shaHits = unfiltered.filter((c) => c.sha.toLowerCase().startsWith(prefix));
